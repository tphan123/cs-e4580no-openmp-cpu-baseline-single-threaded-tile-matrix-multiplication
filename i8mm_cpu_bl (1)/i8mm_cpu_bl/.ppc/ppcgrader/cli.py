...
def select_compiler(compiler):
    if not compiler:
        ...
        return gcc_compiler(str(compiler[1]))
    elif compiler[0] == 'clang':
        return clang_compiler(str(compiler[1]))
    elif compiler[0] == 'nvcc':
        return nvcc_compiler(compiler[1])
    else:
...
def cli(config: Config):
    ...
    identity = f"Grader for {config.code}, version {config.env_version.value}"
    ...
    if args.on_remote is not None and not unknown_args:
        ...
        try:
            ...
            compiler = select_compiler(args["compiler"])
    ...
    # Configure logging (must be before compiler is called)
    reporter = args.reporter(config)
    set_log_level(args.verbose)
    if isinstance(reporter, JsonReporter):
        set_log_enabled(False)

    # Log grader name and version
    reporter.log(identity)
