...
class NvccCompiler(Compiler):
    def __init__(self, program: str = 'nvcc'):
        self.version = NvccCompiler.__get_version(program)
        super().__init__(program=program,
                         common_flags=[
                             '-std=c++17',
                             '--Werror',
    ...
    def __get_version(program: str):
        try:
            ...
            if output.returncode == 0:
                lines = output.stdout.split('\n')
                matches = [
                    re.search(r'release (\d+)\.(\d+)', line) for line in lines
                ]
...
def _scan_for_nvcc(candidates: list):
    best = None
    for program in candidates:
        compiler = NvccCompiler(program)
        if compiler.version and (best is None
                                 or best.version < compiler.version):
            best = compiler
...
def find_nvcc_compiler():
    # first, check if we find nvcc on PATH
    nvcc = _scan_for_nvcc(NVCC_BINARIES)
    if nvcc is None:
        # check for nvidia packages that don't add nvcc to path
        return _scan_for_nvcc(list(
            Path("/usr/local/").glob("cuda-*/bin/nvcc")))
